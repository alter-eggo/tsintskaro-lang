...
Delivery uses one continuous numbered list, including for 100-word batches. It splits only when the text exceeds Telegram's 4096-character message limit, preserving complete entries where possible and every translation in full. The batch heading and discussion deadline appear only in the first message. Each dialect word remains individually copyable. `word_review_batch.messageFormatVersion` preserves the original page boundaries for interrupted deliveries created before this change, so resuming them cannot skip words.

Review start, stop, size changes and extra deliveries run silently, including when no new words are available. Report/quiz configuration, buffer clearing and memory changes also produce no routine success or progress messages. Requested content, explicit status queries and actionable errors still receive replies.

Full words and translations are retained; an exceptionally long entry continues in the next message. The bot persists message IDs and resumes interrupted delivery from the first unsent message. PostgreSQL advisory locking serializes delivery and configuration changes across bot instances. As with any Telegram send, an interruption after Telegram accepts a message but before its ID is saved can require manual reconciliation; Telegram provides no application-supplied idempotency key for this operation.
